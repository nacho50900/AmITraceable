"""
Módulo nuevo (opcional, best-effort): dado el NOMBRE de un edificio o
monumento que Moondream2 dice reconocer en una foto (campo
EDIFICIO_EMBLEMATICO, ver app/vision/scene_analysis.py), le pregunta a
Mistral si reconoce ese lugar con certeza y, si es así, sus coordenadas
reales -- para poder sobreescribir la estimación por similitud visual de
DINOv2 (app/vision/geolocation.py) en esa foto concreta con algo mucho
más preciso que "la foto se parece a otras fotos de esta provincia".

Por qué Mistral y no Moondream2 para las coordenadas: Moondream2 es un
VQA pequeño (cuantizado localmente, ver su nota de fiabilidad en
...
Mistral es un LLM de propósito general con mucho más conocimiento del
mundo real (nombres de monumentos, ciudades, países), así que aquí se usa
su conocimiento general, NO visión -- solo recibe el NOMBRE que propuso
Moondream2 (texto plano), nunca la imagen.

Nunca se usa a ciegas: se le pide explícitamente a Mistral que devuelva
...
ai_attribute_extraction.py): esto envía a Mistral (Mistral AI, UE) SOLO
el nombre de un edificio/monumento propuesto por Moondream2 y, como
contexto opcional para desambiguar nombres genéricos (p. ej. "catedral"),
la descripción general de la foto -- nunca la imagen en sí, nunca datos
personales de la cuenta analizada.
"""
import logging
...
# Umbral de confianza para aceptar la resolución de Mistral y sobreescribir
...
async def resolve_landmark_coordinates(
    ...
    """Punto de entrada del módulo. Devuelve None si Mistral no está
    configurado, no reconoce el lugar con certeza, la confianza queda por
    debajo de MIN_CONFIDENCE_FOR_LANDMARK_OVERRIDE, o la llamada falla por
    cualquier motivo -- nunca lanza excepciones, mismo criterio best-effort
    que el resto de módulos que hablan con Mistral en este proyecto."""
    if not settings.mistral_api_key or not landmark_name.strip():
        return None
    ...
    try:
        parsed = await call_mistral_json(
            _SYSTEM_PROMPT, prompt, max_tokens=200, temperature=0.0
        )
    except (MistralHTTPError, MistralRequestError) as exc:
